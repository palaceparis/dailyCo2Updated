...
class ST_GAT(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_nodes, heads=8, dropout=0.0):
        ...
        # two separate graph attentional layers with 8 attention heads each
        ...
        self.lstm2 = torch.nn.LSTM(
            input_size=lstm1_hidden_size*2, hidden_size=lstm2_hidden_size, num_layers=1)
        for name, param in self.lstm1.named_parameters():
            if 'bias' in name:
                torch.nn.init.constant_(param, 0.0)
            elif 'weight' in name:
        ...
        self.linear = torch.nn.Linear(
            lstm2_hidden_size, self.n_nodes*self.n_pred)
    ...
    def forward(self, data, device):
        ...
        """
        x, edge_index, edge_index_2 = data.x, data.edge_index, data.edge_index_2
        # apply dropout
        if device == 'cpu':
        ...
        # gat layers: output of gat1 and gat2: [11400, 12]
        x1 = self.gat1(x, edge_index)
        x2 = self.gat2(x, edge_index_2)
